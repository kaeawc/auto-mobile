 ...
 * interaction.
 *
 * **Dependency configuration:** This class lives in `src/main/` so the SDK can be published as a
 * single artifact. Apps should declare it as `debugImplementation` to prevent it from shipping in
 * release builds. Using `implementation` instead will include the broadcast receiver in production
 * APKs.
 *
 * ## App integration
 *
 ...
 *    ```
 *
 * 3. In test `@Before` setup, call [clearOverride] to prevent stale overrides from a previous test
 *    from affecting the current one.
 *
 * ## How MCP-triggered overrides work
 *
 * The AutoMobile MCP `biometricAuth` tool broadcasts an override intent via ADB and then triggers
 * the biometric callback using `adb emu finger touch` (emulator). The broadcast is received here
 * and stored. When [consumeOverride] is called inside the app's callback the stored result is
 * returned and the override is cleared.
 */
object AutoMobileBiometrics {
  ...
  /**
   * Initialize with application context. Required to receive MCP broadcast overrides. Called
   * automatically by `AutoMobileSDK.initialize`.
   *
   ...
   *
   * The override expires after [ttlMs] milliseconds to prevent stale values from leaking across
   * test cases. Call [clearOverride] in `@Before` test setup to guarantee a clean state.
   *
   ...
   *
   * Call this in `@Before` test setup so that stale overrides from a previous test cannot affect
   * the current one.
   */
  fun clearOverride() {
   ...
   *
   * This method is atomic: the override is cleared when it is consumed, so it applies to only one
  ...
  private fun registerReceiver() {
    ...
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
        ...
        @Suppress("UnspecifiedRegisterReceiverFlag") ctx.registerReceiver(broadcastReceiver, filter)
      }
   ...
   *
   * Visible for testing: allows unit tests to invoke the receiver logic directly without going
   * through Android's broadcast delivery mechanism.
   */
  @VisibleForTesting
