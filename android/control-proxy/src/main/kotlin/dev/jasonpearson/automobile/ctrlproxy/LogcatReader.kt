 ...
 *
 * Runs `logcat -v threadtime -T 1` to capture only new entries,
 * parses each line with the threadtime format, and invokes [onLogEvent]
 * for every successfully parsed entry.
 *
 * Lifecycle: [start] from onServiceConnected, [stop] from onDestroy.
 * Auto-reconnects if the logcat process dies unexpectedly.
 */
class LogcatReader(
  ...
  companion object {
     ...
     *
     * Example:
     * `04-01 12:34:56.789  1234  5678 D MyTag  : Hello world`
     */
    ...
    private fun parseLevel(letter: String): Int = when (letter) {
      "V" -> 2  // Log.VERBOSE
      "D" -> 3  // Log.DEBUG
      "I" -> 4  // Log.INFO
      "W" -> 5  // Log.WARN
  ...
  /**
   * Start reading logcat on a background thread.
  ...
  /**
   * Parse a single threadtime-formatted logcat line into a [LogEventResponse].
   * Returns null for lines that don't match the expected format (e.g., headers).
   */
