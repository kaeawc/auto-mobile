  ...
  Row(
    ...
    if (showRegexToggle && onRegexToggle != null) {
      Text(
          ...
          fontSize = 10.sp,
          color =
              if (isRegexEnabled) colors.text.normal else colors.text.normal.copy(alpha = 0.35f),
          modifier =
              Modifier.background(
                      if (isRegexEnabled) colors.text.normal.copy(alpha = 0.12f)
    ...
    if (query.isNotEmpty()) {
      Text(
          ...
          modifier =
              Modifier.clickable { onQueryChange("") }
                  .pointerHoverIcon(PointerIcon.Hand)
                  .padding(4.dp),
      )
    }
