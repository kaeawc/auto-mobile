 ...
 *
 * Uses a ConcurrentHashMap for lock-free reads and a per-key Mutex so that
 * concurrent fetches for *different* keys proceed in parallel while duplicate
 * fetches for the *same* key are coalesced.
 *
...
class InMemoryCache<K, V>(
    ...
    /**
     * Returns a cached value if fresh, otherwise calls [fetch] and caches the result.
     * Concurrent calls for the same key will coalesce — only one fetch runs at a time per key.
     */
