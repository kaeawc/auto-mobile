...
/**
 * Badge showing failure count with color indicating severity. Red = has critical/high severity
 * failures Orange = has medium severity failures Gray = no failures
 */
@Composable
fun FailuresBadge(
  failureCount: Int,
  ...
  Box(
    modifier = modifier.size(18.dp).background(backgroundColor, CircleShape),
...
fun FailuresCollapsedContent(
  ...
  Column(
    modifier =
      modifier.fillMaxHeight().padding(vertical = 8.dp).verticalScroll(rememberScrollState()),
...
private fun formatCompactNumber(value: Int): String =
  when {
    ...
    value >= 1_000 ->
      String.format(java.util.Locale.US, "%.1f", value / 1_000.0).removeSuffix(".0") + "k"
    else -> value.toString()
  }
