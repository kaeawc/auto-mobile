...
class StreamingFailuresDataSource(
  ...
  override suspend fun getTimelineData(
    ...
    return try {
      ...
      val previousTotals =
        response.previousPeriodTotals?.let {
  ...
  /**
   * Poll for new notifications since the last poll. Returns only new notifications, updating the
   * cursor internally.
   */
  ...
  /**
   * Create a flow that polls for new notifications at regular intervals. Implements
   * [StreamingFailuresDataSourceInterface.notificationsFlow].
   */
  ...
  /**
   * Create a flow that polls for updated failure groups at regular intervals. Implements
   * [StreamingFailuresDataSourceInterface.failureGroupsFlow].
   */
...
private fun FailureType.toQueryParam(): String =
  when (this) {
...
private fun FailureSeverity.toQueryParam(): String =
  when (this) {
...
private fun FailureNotificationEntry.toModel(): FailureNotification =
  FailureNotification(
    id = id,
