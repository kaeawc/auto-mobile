...
/**
 * Minimum tap target size in dp as per Android accessibility guidelines. Touch targets should be at
 * least 48x48dp for comfortable tapping.
 */
const val MIN_TAP_TARGET_DP = 48

/**
 * Standard phone screen width in dp (used as baseline for density estimation). Most modern phones
 * (Pixel, Samsung Galaxy, etc.) report ~411-412dp width. Using 411dp gives accurate density
 * estimates for xxhdpi (420dpi) devices.
 */
 ...
 *
 * Uses the shorter screen dimension to estimate density, which provides more consistent results
 * across orientation changes (portrait/landscape).
 *
...
/**
 * Check if an element is a non-compliant tap target. An element is non-compliant if it's clickable
 * but smaller than 48x48dp.
 *
...
/**
 * Check if an element has any clickable descendants. Used to filter out container elements that
 * just pass through clicks.
 */
...
/**
 * Find all non-compliant tap targets in a hierarchy. Returns elements that are clickable but
 * smaller than 48x48dp.
 *
 * Filters out:
...
/**
 * Build the path from root to [targetId] using the pre-computed parent map. Returns a list of
 * element IDs from root to target (inclusive). O(depth) instead of O(n) DFS.
 */
