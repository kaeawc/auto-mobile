...
enum class TimelineCategory(val color: Color, val laneIndex: Int, val label: String) {
    ...
    ToolCall(Color(0xFF7E57C2), 0, "ToolCall"),
    Navigation(Color(0xFF66BB6A), 1, "Navigation"),
    Failure(Color(0xFFEF5350), 1, "Failure"),
    Os(Color(0xFFFF7043), 1, "Os"),
    Accessibility(Color(0xFFAB47BC), 1, "Accessibility"),
    Log(Color(0xFF78909C), 2, "Log"),
    Storage(Color(0xFFFFA726), 2, "Storage"),
    Layout(Color(0xFF26C6DA), 2, "Layout"),
    Memory(Color(0xFFD4E157), 2, "Memory"),
    Performance(Color(0xFFEC407A), 2, "Performance"),
}
...
fun buildTimelineSpans(
    ...
    TimelineSpan(
        ...
        endMs = event.timestamp + durationMs,
        category = category,
        isFiltered = activeFilterCategory != null && category.label != activeFilterCategory,
    )
}
