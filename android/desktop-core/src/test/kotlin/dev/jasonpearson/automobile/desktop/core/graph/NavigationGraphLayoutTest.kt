...
class NavigationGraphLayoutTest {
  ...
  fun `zero canvas size returns empty layout`() {
    val summary = NavigationGraphSummary(nodes = listOf(NavigationGraphSummaryNode(1, "Home", 5)))
  ...
  fun `single node is placed at canvas center`() {
    val layout =
      computeGraphLayout(
        summary = NavigationGraphSummary(nodes = listOf(NavigationGraphSummaryNode(1, "Home", 5))),
