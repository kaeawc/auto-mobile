...
/**
 * Listens for file open/close events and attaches a [TestPlanDocumentListener] only to test plan
 * YAML files. This replaces the former [TestPlanValidationService] which registered a global
 * document listener via EditorFactory but was never instantiated (getInstance was never called).
 *
 * Registered as a `projectListeners` entry in plugin.xml so the IntelliJ platform manages its
