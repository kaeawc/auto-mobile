...
- `automobile.daemon.local.project.path`: Path to a local AutoMobile checkout. When set, the runner uses `node <path>/dist/src/index.js` instead of the package fallback. Can also be set via `AUTOMOBILE_DAEMON_LOCAL_PROJECT_PATH` environment variable.
- `automobile.daemon.package.version`: Version used for the `bunx` / `npx` package fallback, producing `@kaeawc/auto-mobile@<version>`. Can also be set via `AUTOMOBILE_DAEMON_PACKAGE_VERSION` or `AUTOMOBILE_VERSION`. If unset, the runner uses its published jar `Implementation-Version`; it does not synthesize `@latest`.
- `automobile.daemon.dismiss.keyboard.after.input`: When `true`, appends `--dismiss-keyboard-after-input` to the daemon command so Android soft keyboard is hidden after each `inputText` (helps CI emulators). **System property only** — no environment variable fallback (this flag is per-Gradle-invocation, not per-environment). Pass via `-Dautomobile.daemon.dismiss.keyboard.after.input=true` on the Gradle CLI or `systemProperty` on the unit test task.
