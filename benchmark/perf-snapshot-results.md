...
1. **Observe overhead is device-noise-level, not window-driven.** With the counterbalanced
   ...
   with host state across runs (e.g. small windows show *negative* Δ on a loaded host), which is
...
2. **5000 ms is the right default window — no change to `DEFAULT_PERF_WINDOW_MS`.** The
   ...
   window holds ~10 samples — the smallest that yields a *genuine* p50/p90/p95/p99 spread with a
