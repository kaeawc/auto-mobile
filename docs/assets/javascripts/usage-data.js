...
(function () {
  ...
  function buildAssetSeries(snapshots) {
    var ordered = snapshots.slice().sort(function (a, b) {
      return a.date < b.date ? -1 : 1;
    });
    var dates = ordered.map(function (s) {
      return s.date;
    });
  ...
  function lineChart(title, dates, seriesList, valueFor) {
    ...
    seriesList.forEach(function (s) {
      s.points.forEach(function (p) {
        if (valueFor(p) != null && valueFor(p) > max) {
          max = valueFor(p);
        }
      });
    });
    ...
    seriesList.forEach(function (s) {
      ...
      s.points.forEach(function (p) {
        ...
        if (prevDate !== null && utcDayDifference(prevDate, p.date) > 1) {
          penUp = true;
        }
