...
`AUTOMOBILE_IOS_SCREEN_CAPTURE_HELPER`, then a repo/package-local Swift build
output, then a sha256-verified prebuilt universal helper downloaded from the
GitHub release and cached under `~/.auto-mobile/screen-capture-helper/` (issue
#4392) — the same delivery pattern as the CtrlProxy APK/IPA and
`automobile-video.jar`, so a normal macOS install needs no Swift toolchain. The
helper Swift package source is also copied into `dist/ios/screen-capture` so a
checkout/package can still build it. ffmpeg is resolved from `AUTOMOBILE_IOS_WEBRTC_FFMPEG` or
