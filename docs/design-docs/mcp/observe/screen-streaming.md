...
| Quality auto-adjustment | Client-side on the desktop mirror: a per-pane controller uses source-side cumulative encoder drops for severe degradation and decoded frame timing for moderate degradation/recovery. This leaves Android repeat frames and iOS idle suppression neutral, then steps the preset down on sustained encoder overload / back up once healthy, applied by re-subscribing. Because a shared per-device capture's encode is fixed by the first subscriber (see Stream Control), this takes effect for a sole subscriber / the next subscribe; reconfiguring a live shared capture is a server-side follow-up. |
