...
- `videoRecording`
  - Params:
    ...
    - `highlights`: optional list of highlight entries to show during recording on Android and iOS simulator. Each entry includes optional `description`, `shape`, and optional `timing` (`startTimeMs`). iOS physical-device live highlights are disabled by default unless `AUTOMOBILE_IOS_LIVE_HIGHLIGHTS=true`.
