...
| Late WHEP viewers and decoder configuration | The coordination server caches the most recent SPS/PPS and a complete IDR access unit (snapshotted from one access unit) and replays them once a viewer is ready, rewriting each subscriber's RTP sequence numbers into one contiguous space so the replay→live seam carries no gap. The publisher also re-injects cached SPS/PPS before every IDR (RFC 6184 §8.4) so each keyframe is a decoder-initialization point. | [RFC 6184 parameter sets and payload parameters](https://www.rfc-editor.org/rfc/rfc6184.html#section-8) and the public [ITU-T H.264 recommendation](https://www.itu.int/rec/T-REC-H.264). |
| Keyframe requests (PLI) | A WHEP viewer's Picture Loss Indication is relayed by the coordination server to the publisher (throttled), which asks the capture source for a fresh IDR: the persistent encoder via a socket command (`MediaCodec` `PARAMETER_KEY_REQUEST_SYNC_FRAME`), screenrecord via a segment rotation, and iOS via a bounded GOP. | [RTCP PLI (RFC 4585 section 6.3.1)](https://www.rfc-editor.org/rfc/rfc4585.html#section-6.3.1) and [RFC 5104](https://www.rfc-editor.org/rfc/rfc5104.html). |
...
- **Keyframe replay policy:** replaying a cached SPS/PPS + IDR snapshot is chosen
  for fast late-viewer startup; per-subscriber sequence rewriting keeps the
  replay→live seam gap-free. RTCP PLI is now relayed end to end (viewer → server
  → publisher → capture source) rather than deferred.
