...
| ⚙️ <code>getDeviceState</code> / ⚙️ <code>setDeviceState</code>                | Reads or changes Do Not Disturb, simulator biometric enrollment, and network condition. Degraded profiles are best-effort cellular shaping on an Android emulator (`adb emu network …` plus a best-effort Wi-Fi disable), reported `partial` — they may not affect Wi-Fi or app traffic; `offline`/reset are the reliable cases. Unsupported on physical Android and all iOS. |
