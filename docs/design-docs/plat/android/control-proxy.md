...
```bash
adb shell appops set dev.jasonpearson.automobile.ctrlproxy \
...
```bash
adb shell dpm set-device-owner dev.jasonpearson.automobile.ctrlproxy/.AutoMobileDeviceAdminReceiver
```
...
The settings-toggle API lives in `src/utils/CtrlProxyManager.ts` and is implemented by `AndroidCtrlProxyManager`.
