...
> for measuring the _app under test's_ real performance on the iOS Simulator, and
...
**not available on the Simulator**, and any Metal/GPU timing that _does_ record
...
1. **Real fps/jank = in-app SDK, on device and simulator alike.** An AutoMobile
   ...
   source that measures the _app's_ rendering on the Simulator. Caveat: on the
