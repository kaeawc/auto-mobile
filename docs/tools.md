...
| ❌ <code>killDevice</code> / 🧹 <code>deleteDevice</code>                      | Stops a device, or stops and permanently deletes it. Both accept `force: true`, which skips the emulator-console AVD-name confirmation for a wedged Android emulator and acts on the serial as given; it does not bypass the refusals raised when the pooled entry was retired and replaced mid-action, or when no booted target can be identified at all.                                                                                                                             |
