...
| `AUTOMOBILE_IOS_SCREEN_CAPTURE_HELPER` | Absolute path to a `screen-capture-helper` binary for iOS WebRTC capture. Set it to point at a custom or locally built helper; it takes precedence over the bundled prebuilt helper. | the prebuilt universal helper bundled at `dist/vendor/screen-capture-helper/darwin-universal/` when present, else the `swift build` output under `ios/screen-capture/.build` from a checkout |
