...
| `AUTOMOBILE_OBSERVE_PERF_SNAPSHOT` | Opt-in (`1`/`true`/`yes`) to attach a windowed performance snapshot (`perfSnapshot`) to `observe` results — fps percentiles, jank, touch latency, CPU, memory. Off by default; enabling it starts continuous per-device sampling. |
| `AUTOMOBILE_OBSERVE_PERF_WINDOW_MS` | Rolling window (ms) for `perfSnapshot`. Default `5000`, clamped to `1000`–`30000`. |
