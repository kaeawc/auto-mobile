...
| `AUTOMOBILE_IOS_SCREEN_CAPTURE_HELPER` | Absolute path to a `screen-capture-helper` binary for iOS WebRTC capture. Set it to override the prebuilt helper for local development or a custom build. A normal macOS install needs no Swift toolchain — the helper ships prebuilt. | prebuilt universal helper at `dist/vendor/screen-capture-helper/darwin-universal/` on supported macOS installs; on a repo checkout, the `swift build` output under `ios/screen-capture/.build` |
