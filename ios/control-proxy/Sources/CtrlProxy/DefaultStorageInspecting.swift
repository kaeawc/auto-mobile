...
/// Default implementation of StorageInspecting using UserDefaults directly.
/// Provides read/write access to UserDefaults suites for storage inspection.
public final class DefaultStorageInspecting: StorageInspecting {
    private var registeredSuites: [String] = []

    public init() {}

    /// Register a custom suite name for discovery.
    public func registerSuite(_ name: String) {
        if !registeredSuites.contains(name) {
            registeredSuites.append(name)
        }
    }
    ...
    public func listSuites() -> [StorageSuiteInfo] {
        ...
        // Registered custom suites
        for name in registeredSuites {
            if let defaults = UserDefaults(suiteName: name) {
                suites.append(StorageSuiteInfo(
                    name: name,
                    displayName: name,
                    entryCount: defaults.dictionaryRepresentation().count
                ))
            }
        }
