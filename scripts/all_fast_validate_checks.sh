...
add_check "docs-assets" "bun \"$PROJECT_ROOT/scripts/docs/build-docs-assets.ts\" --check" "docs,typescript" "Keep docs/assets/javascripts/*.js in sync with their docs-assets/src TypeScript sources"
...
add_check "lychee-offline" "LYCHEE_REQUIRE_SITE=true \"$PROJECT_ROOT/scripts/lychee/validate_lychee.sh\" --offline" "docs,links" "Validate documentation links offline (local targets + #fragments)"
...
add_check "github-python-lock" "cd \"$PROJECT_ROOT/scripts/github\" && uv sync --locked --no-install-project" "config,dependencies" "Validate the GitHub documentation Python lockfile"
