...
const EXECUTION_FUNCTIONS = new Set(["spawn", "spawnSync", "exec", "execSync", "execFile", "execFileSync"]);
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier) && CHILD_PROCESS_MODULES.has(node.moduleSpecifier.text)) {
      ...
      if (bindings && ts.isNamedImports(bindings)) {
        for (const specifier of bindings.elements) {
          ...
          if (EXECUTION_FUNCTIONS.has(imported)) { importedExecutors.add(specifier.name.text); }
        }
      }
    }
    ...
    if (ts.isVariableDeclaration(node) && node.initializer) {
      if (ts.isIdentifier(node.name)) {
        if (isChildProcessRequire(node.initializer)) { namespaces.add(node.name.text); }
        ...
        if (ts.isPropertyAccessExpression(node.initializer) &&
          ts.isIdentifier(node.initializer.expression) && namespaces.has(node.initializer.expression.text) &&
    ...
    if (ts.isCallExpression(node) && !isDiagnosticProcessTableCall(file, node)) {
      ...
      const namespaced = ts.isPropertyAccessExpression(expression) &&
      ...
      if (direct || namespaced) { record(node); }
    }
