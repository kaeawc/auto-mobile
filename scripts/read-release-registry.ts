...
export async function readReleaseRegistryField(
  ...
  const entry = version
    ? version === "nightly" ? release.NIGHTLY_CHECKSUM_ENTRY : release.RELEASE_CHECKSUM_REGISTRY?.find(candidate => candidate.version === version)
...
async function main(): Promise<void> {
  ...
  if (!field || !sourcePath) {
    throw new Error("Usage: bun scripts/read-release-registry.ts <field> <release.ts path> [version]");
  }
