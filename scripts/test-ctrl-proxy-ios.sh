#!/bin/bash
#
# CtrlProxy iOS Diagnostic Script
...
# =============================================================================
# Test 1: Check if CtrlProxy project exists and is buildable
# =============================================================================
echo -e "${BLUE}[1/5] Checking CtrlProxy iOS Installation${NC}"
echo ""

# Check Xcode project exists
if [ -d "${CTRL_PROXY_IOS_DIR}/CtrlProxy.xcodeproj" ]; then
    print_status 0 "Xcode project found: ios/control-proxy/CtrlProxy.xcodeproj"
else
    print_status 1 "Xcode project not found at ios/control-proxy/CtrlProxy.xcodeproj"
fi
...
if [ -f "${CTRL_PROXY_IOS_DIR}/Tests/CtrlProxyUITests/CtrlProxyUITests.swift" ]; then
    print_status 0 "CtrlProxyUITests.swift found"
else
    print_status 1 "CtrlProxyUITests.swift not found"
fi
...
# First check if xcodebuild.*CtrlProxy
XCTEST_PROCESS_PID=$(pgrep -f 'xcodebuild.*CtrlProxy' 2>/dev/null || echo "")
if [ -n "$XCTEST_PROCESS_PID" ]; then
    print_status 0 "CtrlProxy iOS xcodebuild process found (PID: ${XCTEST_PROCESS_PID})"
    PROCESS_RUNNING=true
else
    print_warning "No CtrlProxy iOS xcodebuild process found"
...
if [ "$HEALTH_RESPONSE" == "FAILED" ]; then
    ...
    if [ "$PROCESS_RUNNING" = false ]; then
        echo ""
        print_info "CtrlProxy iOS is not running. Attempting to start it..."
...
echo "      - request_two_finger_swipe   : Two-finger swipe"
echo "      - request_multi_finger_swipe : Multi-finger swipe"
...
BUN_SCRIPT
    ...
    echo ""
    print_info "To start CtrlProxy iOS:"
    echo ""
