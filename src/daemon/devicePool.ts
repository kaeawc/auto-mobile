 ...
 */
/**
 * Freshness class of a discovery observation feeding mutable device metadata.
 * `refresh` is the pool's own ordered discovery sweep; `snapshot` is a
 * caller-supplied `BootedDevice` of unknown age taken before the assignment
 * mutex was acquired.
 */
  ...
  async reserveAndroidStartupLease(
    ...
    for (;;) {
      ...
      await this.assignmentMutex.runExclusive(() => {
        ...
        matchingRecoveryAvdNames = Array.from(this.recoveringAndroidImages.keys()).filter(
