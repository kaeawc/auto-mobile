...
function createDeadlineSignal(
  ...
  const inheritedSignal = combineWithAmbientAbort(options.signal);
  if (deadlineMs === undefined) {
    return { signal: inheritedSignal, dispose: () => {} };
  }

  const controller = new AbortController();
  const signal = combineAbortSignals(inheritedSignal, controller.signal) ?? controller.signal;
  const remainingMs = deadlineMs - timer.now();
  if (remainingMs <= 0) {
    controller.abort(new DoctorDeadlineError());
    return { signal, dispose: () => {} };
  }
  const timeout = timer.setTimeout(() => controller.abort(new DoctorDeadlineError()), remainingMs);
  return {
    signal,
    dispose: () => timer.clearTimeout(timeout),
  };
}
