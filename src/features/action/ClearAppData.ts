...
export class ClearAppData {
  ...
  async execute(
    ...
    switch (this.device.platform) {
      ...
      default:
        throw new ActionableError(`Clear app data is not supported for platform '${this.device.platform}'`);
    }
  }
  ...
  private async executeAndroid(
    ...
    const targetUserId = await perf.track("detectTargetUser", async () => {
      return (await new AndroidUserTargetResolver(adb).resolve({
        packageName,
  ...
  private async executeIos(bundleId: string): Promise<ClearAppDataResult> {
    ...
    const isSimulator = this.isSimulatorOverride ?? (() => isIosSimulatorUdid(this.device.deviceId));
    return isSimulator()
      ? this.clearIosSimulator(simctl, bundleId)
      : this.clearIosPhysical(bundleId);
  }

  private async clearIosSimulator(simctl: SimCtlClient, bundleId: string): Promise<ClearAppDataResult> {
  ...
  private async clearIosPhysical(bundleId: string): Promise<ClearAppDataResult> {
    ...
    try {
      ...
      logger.warn(`[iOS] Failed to clear app data for ${bundleId} via reinstall: ${errorMessage(error)}`);
      return { success: false, packageName: bundleId, error: errorMessage(error) };
    }
  }
}
