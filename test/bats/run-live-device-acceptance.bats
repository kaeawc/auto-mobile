...
printf 'timeout %s\n' "$*" >> "${COMMAND_LOG}"
shift 3 # -k 2 <seconds>
if [[ "${TIMEOUT_BUILD:-}" == "1" && "$*" == "bun run build" ]]; then
  exit 124
fi
...
@test "runs the full Android then iOS matrix with one private run scope and exact required arguments" {
  ...
  grep -q '^timeout -k 2 28 bun run build$' "${COMMAND_LOG}"
  ...
  [ "$(grep -c '^timeout -k 2 ' "${COMMAND_LOG}")" -eq 3 ]
...
@test "bounds the build within the advertised total deadline" {
  run env AUTOMOBILE_ACCEPTANCE_LIVE=1 TIMEOUT_BUILD=1 PATH="${MOCK_BIN}:${PATH}" bash "${SCRIPT}" \
    --confirm-live \
    --test-owned-devices \
    --android-avd-name "acceptance-android" \
    --android-sibling-avd-name "acceptance-android-sibling" \
