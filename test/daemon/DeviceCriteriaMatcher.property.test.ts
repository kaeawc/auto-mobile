  ...
  test("Android rediscovery requires the transport and accepts only matching or unresolved AVD names", () => {
    fc.assert(
      fc.property(
        ...
        shortString,
        shortString,
        (expectedDeviceId, expectedAvdName, candidateDeviceId, candidateAvdName) => {
          const candidate = {
          ...
          return (
            matcher.androidRediscoveryMatches(candidate, expectedDeviceId, expectedAvdName) ===
  ...
  test("capacity includes exactly the pending recoveries eligible for the platform", () => {
    fc.assert(
      fc.property(
        ...
        count,
        platform,
        (pooledDeviceCount, pendingRecoveryCount, requiredCount, requestedPlatform) => {
          const eligibleRecoveries =
            requestedPlatform === undefined || requestedPlatform === "android"
              ? pendingRecoveryCount
              : 0;
          return (
            matcher.hasSufficientCapacityIncludingAndroidRecovery(
              pooledDeviceCount,
              requiredCount,
              pendingRecoveryCount,
              requestedPlatform,
  ...
  test("pending-image matching is existential and honors normalized AVD names", () => {
    fc.assert(
      fc.property(fc.array(shortString, { maxLength: 20 }), shortString, (names, requestedName) => {
        ...
        return (
          matcher.someDeviceImageMatchesCriteria(images, {
            platform: "android",
