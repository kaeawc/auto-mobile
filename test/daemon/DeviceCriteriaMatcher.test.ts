...
const deviceImage = (overrides: Partial<DeviceInfo> & { name: string; platform: Platform }): DeviceInfo => ({
  isRunning: false,
  ...overrides,
});
...
describe("DeviceCriteriaMatcher", () => {
  ...
  describe("filterDevices", () => {
    const devices = [
      ...
      pooledDevice({ id: "sim-1", platform: "ios", name: "iPhone 15 Pro", iosVersion: "17.5", simulatorType: "iPhone 15 Pro" }),
      pooledDevice({ id: "sim-2", platform: "ios", name: "iPhone 15", iosVersion: "17.4", simulatorType: "iPhone 15" }),
    ];
    ...
    test("matches simulator type case-insensitively against name or simulatorType", () => {
      const result = matcher.filterDevices(devices, { platform: "ios", simulatorType: "iphone 15 pro" });
  ...
  describe("deviceImageMatchesCriteria", () => {
    test("matches simulator type via deviceType identifier suffix", () => {
      ...
      expect(matcher.deviceImageMatchesCriteria(image, { simulatorType: "iPhone 15 Pro" })).toBe(true);
    });
  ...
  describe("isStartableDeviceImage", () => {
    test("rejects unavailable images", () => {
      expect(matcher.isStartableDeviceImage(deviceImage({ name: "x", platform: "ios", deviceId: "u", isAvailable: false }))).toBe(false);
    });

    test("requires a deviceId and Shutdown state for iOS", () => {
      expect(matcher.isStartableDeviceImage(deviceImage({ name: "x", platform: "ios" }))).toBe(false);
      expect(matcher.isStartableDeviceImage(deviceImage({ name: "x", platform: "ios", deviceId: "u", state: "Booted" }))).toBe(false);
      expect(matcher.isStartableDeviceImage(deviceImage({ name: "x", platform: "ios", deviceId: "u", state: "Shutdown" }))).toBe(true);
    });

    test("accepts android images without iOS constraints", () => {
      expect(matcher.isStartableDeviceImage(deviceImage({ name: "Pixel", platform: "android" }))).toBe(true);
    });
  });

  describe("getDeviceImageKey", () => {
    test("prefers deviceId, falling back to platform:name", () => {
      expect(matcher.getDeviceImageKey(deviceImage({ name: "x", platform: "ios", deviceId: "udid" }))).toBe("udid");
      expect(matcher.getDeviceImageKey(deviceImage({ name: "Pixel", platform: "android" }))).toBe("android:Pixel");
    });
  });

  describe("withDeviceImageMetadata", () => {
    test("backfills missing fields from the image without overwriting booted values", () => {
      const ready: BootedDevice = { name: "sim", platform: "ios", deviceId: "udid", iosVersion: "17.5" };
      const image = deviceImage({
        name: "iPhone 15 Pro",
        platform: "ios",
        deviceType: "com.apple.CoreSimulator.SimDeviceType.iPhone-15-Pro",
  ...
  describe("getBootedDeviceSimulatorType", () => {
    test("prefers explicit simulatorType then derives from deviceType", () => {
      expect(matcher.getBootedDeviceSimulatorType({ name: "s", platform: "ios", deviceId: "u", simulatorType: "iPhone 15" } as BootedDevice)).toBe("iPhone 15");
      expect(matcher.getBootedDeviceSimulatorType({ name: "s", platform: "ios", deviceId: "u", deviceType: "com.apple.CoreSimulator.SimDeviceType.iPhone-15-Pro" } as BootedDevice)).toBe("iPhone 15 Pro");
      expect(matcher.getBootedDeviceSimulatorType({ name: "s", platform: "android", deviceId: "u" })).toBeUndefined();
    });
  });
  ...
  describe("formatCriteriaSummary", () => {
    ...
    test("joins present criteria fields", () => {
      expect(matcher.formatCriteriaSummary({ platform: "ios", simulatorType: "iPhone 15", iosVersion: "17.5" }))
