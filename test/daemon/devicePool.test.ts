...
describe("DevicePool", () => {
  ...
  class StubbornRecoveryChildProcess extends FakeChildProcess {
    readonly signals: Array<NodeJS.Signals | number | undefined> = [];
    readonly sigtermSent = Promise.withResolvers<void>();
    readonly sigkillSent = Promise.withResolvers<void>();
    ...
    override kill(signal?: NodeJS.Signals | number): boolean {
      ...
      this.signals.push(signal);
      if (signal === "SIGTERM") {
        this.sigtermSent.resolve();
      } else if (signal === "SIGKILL") {
        this.sigkillSent.resolve();
      }
      return true;
    }
  }
  ...
  test("rejects an unpooled Unknown Android replacement whose serial is recovering", async () => {
    ...
    await expect(
      devicePool.reserveDeviceForReadiness(
        replacement.deviceId,
        replacement,
        recoveringImage.name,
        undefined,
        undefined,
        true,
      ),
  ...
  test("rejects an anonymous re-acquisition of a freshly started device", async () => {
    const device = createBootedDevice("emulator-5554", "android", "Pixel 8");
    const sourceImage: DeviceInfo = {
      name: "Pixel 8",
      platform: "android",
    ...
    await expect(
      ...
      "Freshly started device 'emulator-5554' was assigned to session " +
        "owner-session before its owning session could reserve it.",
    );
    expect(devicePool.getDevice(device.deviceId)?.sessionId).toBe("owner-session");
  });
  ...
  describe("intentional-shutdown marker incarnation scoping", () => {
    ...
    test("mints a new device session epoch when a serial disappears and returns", async () => {
      ...
      expect(added).toBe(1);
      expect(devicePool.getDevice(reconnected.deviceId)?.incarnation).toBeGreaterThan(
        firstIncarnation,
      );
    ...
    test("reserves a verified AVD name when the running emulator lacks AVD metadata", async () => {
      const original = createBootedDevice("emulator-5554", "android", "Unknown (emulator-5554)");
    ...
    test("keeps a same-serial recovery replacement reserved for its readiness owner", async () => {
      ...
      try {
        expect(devicePool.getDevice(replacement.deviceId)?.incarnation).not.toBe(
          originalIncarnation,
        );
        await expect(
          devicePool.bindOrReuseDeviceSession(
  ...
  describe("refreshDevices", () => {
    ...
    test("resets device-scoped session state when recovery reuses the serial", async () => {
      ...
      sessionManager.onSessionDeviceUnbound((_sessionId, deviceId) =>
        unboundDevices.push(deviceId),
      );
      const shutdownReservation = await devicePool.reserveDeviceForShutdown(device.deviceId);
      if (!shutdownReservation) {
        throw new Error("expected shutdown reservation");
    ...
    test("does not retire a same-serial successor while cleaning up a recovery replacement", async () => {
      ...
      try {
        ...
        await devicePool.removeDevice(replacement.deviceId, false, handoff.replacementDevice);
        fakeDeviceManager.bootedDevices = [replacement];
        await devicePool.addDevice(replacement, sourceImage);
        await devicePool.bindOrReuseDeviceSession(
          "successor-session",
        ...
        expect(
          await devicePool.retireDeviceAfterSystemUiAnrRecoveryFailure(handoff.replacementDevice),
        ).toBe(false);
        expect(devicePool.getDevice(replacement.deviceId)).toMatchObject({
  ...
  describe("assignDeviceToSession", () => {
    ...
    test("does not reserve a second device when binding an automatically created session", async () => {
      ...
      const otherDeviceId =
        session.assignedDevice === "emulator-5554" ? "emulator-5556" : "emulator-5554";

      await expect(
        devicePool.bindOrReuseDeviceSession("shared-session", otherDeviceId, "android"),
      ).rejects.toThrow(
        `Session 'shared-session' is already assigned to device '${session.assignedDevice}'.`,
      );
  ...
  describe("assignMultipleDevices", () => {
    ...
    test("preserves a replacement session that reuses a UUID during rollback", async () => {
      ...
      expect(devicePool.getDevice("sim-1")).toMatchObject({
        sessionId: "session-a",
        status: "busy",
      });
      expect(devicePool.getDevice("sim-2")).toMatchObject({ sessionId: null, status: "idle" });
    ...
    test("releases the old device without rolling back a replacement session with the same UUID", async () => {
      ...
      const allocation = devicePool.assignMultipleDevices(
        ["session-a", "session-b"],
        1_000,
        "android",
      );
  ...
  describe("assignMultipleDevices", () => {
    ...
    test("quarantines a process-exit session before incident persistence completes", async () => {
      ...
      expect(recordedIncidents).toMatchObject([{ recovery: { outcome: "not-attempted" } }]);
    ...
    test("keeps process-exit quarantine while eviction claims recovery", async () => {
      ...
      const eviction = internals.evictStartedDeviceAfterProcessExit("emulator-5554", 1, null);
    ...
    test("coalesces concurrent loss signals into one session-preserving recovery", async () => {
      ...
      await expect(Promise.all([first, duplicate])).resolves.toEqual(["recovered", "recovered"]);
    ...
    test("holds the recovery lease until a preempted emulator process exits", async () => {
      const originalRebootOnDeath = process.env.AUTOMOBILE_ANDROID_REBOOT_ON_DEATH;
      process.env.AUTOMOBILE_ANDROID_REBOOT_ON_DEATH = "1";
      ...
      try {
        ...
        const teardown = lifecycleCoordinator
          .reserve(
            { kind: "stable", platform: "android", stableId: "Pixel 8" },
        ...
        expect(manager.recoveryChild.signals).toEqual(["SIGTERM"]);
        ...
        expect(teardownAcquired).toBe(true);
      } finally {
        manager.releaseRecovery();
        if (originalRebootOnDeath === undefined) {
          delete process.env.AUTOMOBILE_ANDROID_REBOOT_ON_DEATH;
    ...
    test("retains a preempted recovery lease after bounded stop failure until late exit", async () => {
      const originalRebootOnDeath = process.env.AUTOMOBILE_ANDROID_REBOOT_ON_DEATH;
      process.env.AUTOMOBILE_ANDROID_REBOOT_ON_DEATH = "1";
      const manager = new DeferredRecoveryDeviceManagerWithStubbornChild();
      const lifecycleCoordinator = new InMemoryVirtualDeviceLifecycleCoordinator(fakeTimer);
    ...
    test("does not relaunch an AVD while its old emulator process survives SIGKILL", async () => {
      ...
      try {
        ...
        const teardown = lifecycleCoordinator.reserve(
          { kind: "stable", platform: "android", stableId: "Pixel 8" },
          { operation: "teardown", deadlineMs: fakeTimer.now() + 1_000 },
        );
        let teardownAcquired = false;
        void teardown.then(() => {
          teardownAcquired = true;
    ...
    test("retains a failed pool-start lease until the stubborn emulator exits", async () => {
      const image: DeviceInfo = {
        name: "Pixel 8",
        platform: "android",
        isRunning: false,
  ...
  describe("unresolved emulator identity quarantine", () => {
    ...
    test("re-checks identity at the paused session-release commit", async () => {
      const device = stamped(poolDevice("emulator-5554", "Pixel_8_API_35"), 1);
      const replacement = stamped(poolDevice("emulator-5554", "Pixel_7_API_34"), 2);
      ...
      try {
        ...
        // The replacement was accepted before release started, but this newer
        // confirmation arrives while the releaser is paused before it commits.
        await devicePool.reconcileDiscoveryObservation([stamped(device, 3)], "test:newer");
        ...
        expect(devicePool.getDevice(device.deviceId)).toBe(pooled);
        expect(pooled.sessionId).toBe("owner-session");
        expect(sessionManager.getSession("owner-session")).not.toBeNull();
      } finally {
        commitRelease.resolve();
      }
    });
    ...
    test("does not let a stale resolved observation outrank newer unresolved replacement evidence", async () => {
      ...
      try {
        ...
        expect(installed?.identityObservedAt).toBe(4);
    ...
    test("lets a later resolved replacement clear pending unresolved evidence", async () => {
      const device = stamped(poolDevice("emulator-5554", "Pixel_8_API_35"), 1);
      const replacement = stamped(poolDevice("emulator-5554", "Pixel_7_API_34"), 2);
      const unresolvedReplacement = stamped(unresolved("emulator-5554"), 4);
      const resolvedReplacement = stamped(poolDevice("emulator-5554", "Pixel_6_API_33"), 5);
      await initializeLiveDevices([device]);
      await devicePool.reconcileDiscoveryObservation([device], "test:initial");
      const pooled = devicePool.getDevice(device.deviceId);
      if (!pooled) {
