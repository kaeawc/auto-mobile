...
describe("SessionManager", () => {
  ...
  test("acknowledges a liveness ownership claim only after durable persistence", async () => {
    ...
    try {
      ...
      const claim = manager.claimLivenessOwnership("session-1", "owner-token").then((result) => {
        acknowledged = true;
        return result;
      });
      await persistenceStarted.promise;
  ...
  test("serializes concurrent ownership claims so a failed older write cannot roll back its successor", async () => {
    const firstPersistenceStarted = Promise.withResolvers<void>();
    const finishFirstPersistence = Promise.withResolvers<void>();
    const persistedTokens: string[] = [];
    const manager = new SessionManager(fakeTimer, {
      async upsertActiveSession() {},
      async recordActivity() {},
      async recordLivenessOwnership(_sessionUuid, ownerToken) {
        if (ownerToken === "owner-a") {
          firstPersistenceStarted.resolve();
          await finishFirstPersistence.promise;
          throw new Error("owner-a persistence failed");
        }
        if (ownerToken) {
    ...
    try {
      ...
      const secondClaim = manager.claimLivenessOwnership("session-1", "owner-b").finally(() => {
        secondSettled = true;
      });
      await Promise.resolve();
  ...
  test("restores persisted iOS ownership before a replacement claim", async () => {
    const persisted: DeviceSession = {
      ...
      heartbeat_timeout_ms: 5,
      has_received_heartbeat: 1,
      liveness_owner_token: "current-owner-token",
  ...
  test("bounds a hung rehydration and skips the remaining rows", async () => {
    ...
    try {
      ...
      await expect(rehydration).resolves.toEqual({
        ...
        terminalized: [],
        skipped: [
          { sessionUuid: "hung-rehydration", reason: "startup-deadline" },
          { sessionUuid: "queued-rehydration", reason: "startup-deadline" },
        ],
        timedOut: true,
      });
      expect(assignments).toBe(1);
  ...
  test("does not time out when all persisted rows recover quickly", async () => {
    const persistence = new FakeDeviceSessionPersistence();
    for (const sessionUuid of ["quick-rehydration-a", "quick-rehydration-b"]) {
      await persistence.upsertActiveSession({
        sessionUuid,
        deviceId: "emulator-5554",
        stableDeviceId: "Pixel_8_API_35",
    ...
    try {
      await expect(
        restarted.rehydratePersistedSessions(devicePool, { deadlineMs: 15_000 }),
      ).resolves.toEqual({
        rehydrated: ["quick-rehydration-a", "quick-rehydration-b"],
        terminalized: [],
        skipped: [],
        timedOut: false,
      });
  ...
  test("does not attempt recovery when the startup deadline is zero", async () => {
    const persistence = new FakeDeviceSessionPersistence();
    await persistence.upsertActiveSession({
      sessionUuid: "zero-deadline-rehydration",
      deviceId: "emulator-5554",
      stableDeviceId: "Pixel_8_API_35",
      platform: "android",
      createdAtMs: 1,
    ...
    await persistence.markReleased("zero-deadline-rehydration", "released", 30, "daemon-restart");
    const restarted = new SessionManager(fakeTimer, persistence);
    let assignments = 0;
    try {
      await expect(
        ...
        rehydrated: [],
        terminalized: [],
        skipped: [{ sessionUuid: "zero-deadline-rehydration", reason: "startup-deadline" }],
        timedOut: true,
      });
      expect(assignments).toBe(0);
    } finally {
  ...
  test("does not rehydrate terminal persisted rows", async () => {
    ...
    try {
      ...
      ).resolves.toEqual({ rehydrated: [], terminalized: [], skipped: [], timedOut: false });
      expect(assignments).toBe(0);
    } finally {
      restarted.stopCleanupTimer();
    }
