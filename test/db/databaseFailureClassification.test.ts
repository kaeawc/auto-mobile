...
describe("classifyDatabaseFailure", () => {
  test.each<[string, unknown, DatabaseFailureKind]>([
    ...
    ["resource temporarily unavailable", new Error("resource temporarily unavailable"), "transient"],
    ...
    ["deterministic migration throw", new Error("migration 0007 failed: column already exists"), "permanent"],
    ["incomplete extraction (constructed)", createIncompleteExtractionError("kysely"), "permanent"],
    ["cannot find package", new Error("Cannot find package 'kysely' from '/tmp/x/m.ts'"), "permanent"],
    ...
    ["EBUSY code, unrelated message", Object.assign(new Error("startup failed"), { code: "EBUSY" }), "transient"],
    ["SQLITE_BUSY code, unrelated message", Object.assign(new Error("boot error"), { code: "SQLITE_BUSY" }), "transient"],
    ["EAGAIN code, unrelated message", Object.assign(new Error("nope"), { code: "EAGAIN" }), "transient"],
    ["ENOSPC code, unrelated message", Object.assign(new Error("boot error"), { code: "ENOSPC" }), "permanent"],

    // A non-string code is ignored; classification falls back to the message.
    ["numeric code ignored, message is transient", Object.assign(new Error("database is locked"), { code: 42 }), "transient"],
    ["numeric code ignored, message is permanent", Object.assign(new Error("boot error"), { code: 42 }), "permanent"],
