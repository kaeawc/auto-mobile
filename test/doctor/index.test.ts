...
describe("runDoctor", () => {
  ...
  test("forwards ambient request cancellation to a downstream probe without a timeout", async () => {
    const ambient = new AbortController();
    let markStarted: (signal: AbortSignal) => void = () => {};
    const started = new Promise<AbortSignal>((resolve) => {
      markStarted = resolve;
    });
    const doctor = runWithAbortSignal(ambient.signal, () =>
      runDoctor(
        { android: true },
        {
          runSystemChecks: () => [],
          runAndroidChecks: async (options) =>
            await new Promise<CheckResult[]>((_resolve, reject) => {
              const signal = options.signal;
              if (!signal) {
                reject(new Error("Expected the ambient request signal"));
                return;
              }
              markStarted(signal);
    ...
    expect(await started).toBe(ambient.signal);
    const disconnect = new Error("daemon request disconnected");
    ambient.abort(disconnect);

    await expect(doctor).rejects.toBe(disconnect);
  });
