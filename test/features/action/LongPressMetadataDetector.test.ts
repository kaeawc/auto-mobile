...
describe("LongPressMetadataDetector", () => {
  ...
  test("detects a context menu appearing as a new root with menu indicators", () => {
    ...
    const after = node(
      ...
      [node({ "resource-id": "menu_item_copy", "class": "TextView", "text": "Copy", "bounds": "[10,10][50,30]" })]
    );
  ...
  test("detects text selection when a selection range becomes active", () => {
    const before = node({ "resource-id": "field", "class": "EditText", "bounds": "[0,0][100,40]", "text": "hello" });
    const after = node({
      "resource-id": "field",
      "class": "EditText",
      "bounds": "[0,0][100,40]",
