...
import { buildAsciiKeyEventPlan, type KeyEventPlan } from "../../../src/features/action/asciiKeyEvents";
...
const DIRECT_CODES: Record<string, string> = {
  " ": "KEYCODE_SPACE", "-": "KEYCODE_MINUS", "=": "KEYCODE_EQUALS", "[": "KEYCODE_LEFT_BRACKET",
  "]": "KEYCODE_RIGHT_BRACKET", "\\": "KEYCODE_BACKSLASH", ";": "KEYCODE_SEMICOLON", "'": "KEYCODE_APOSTROPHE",
  ",": "KEYCODE_COMMA", ".": "KEYCODE_PERIOD", "/": "KEYCODE_SLASH", "`": "KEYCODE_GRAVE", "@": "KEYCODE_AT"
};
const SHIFTED_CODES: Record<string, string> = {
  "!": "KEYCODE_1", "#": "KEYCODE_3", "$": "KEYCODE_4", "%": "KEYCODE_5", "^": "KEYCODE_6", "&": "KEYCODE_7",
  "*": "KEYCODE_8", "(": "KEYCODE_9", ")": "KEYCODE_0", "_": "KEYCODE_MINUS", "+": "KEYCODE_EQUALS",
  "{": "KEYCODE_LEFT_BRACKET", "}": "KEYCODE_RIGHT_BRACKET", "|": "KEYCODE_BACKSLASH", ":": "KEYCODE_SEMICOLON",
  "\"": "KEYCODE_APOSTROPHE", "<": "KEYCODE_COMMA", ">": "KEYCODE_PERIOD", "?": "KEYCODE_SLASH", "~": "KEYCODE_GRAVE"
};
...
describe("buildAsciiKeyEventPlan (property-based)", () => {
  test("lowercase maps to a capability-independent uppercase keyevent", () => {
    fc.assert(
      fc.property(lower, fc.boolean(), (c, supports) =>
        eqPlan(buildAsciiKeyEventPlan(c, supports), { commands: [`shell input keyevent KEYCODE_${c.toUpperCase()}`] })
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("digits map to a capability-independent keyevent", () => {
    fc.assert(
      fc.property(digit, fc.boolean(), (c, supports) =>
        eqPlan(buildAsciiKeyEventPlan(c, supports), { commands: [`shell input keyevent KEYCODE_${c}`] })
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("direct symbols map to their keyevent, independent of capability", () => {
    fc.assert(
      fc.property(directSym, fc.boolean(), (c, supports) =>
        eqPlan(buildAsciiKeyEventPlan(c, supports), { commands: [`shell input keyevent ${DIRECT_CODES[c]}`] })
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("uppercase needs a SHIFT chord — a plan iff keycombination is supported", () => {
    fc.assert(
      fc.property(upper, c => {
        return (
          eqPlan(buildAsciiKeyEventPlan(c, true), { commands: [`shell input keycombination KEYCODE_SHIFT_LEFT KEYCODE_${c}`] }) &&
  ...
  test("shifted symbols need a SHIFT chord — a plan iff keycombination is supported", () => {
    fc.assert(
      fc.property(shiftedSym, c => {
        return (
          eqPlan(buildAsciiKeyEventPlan(c, true), { commands: [`shell input keycombination KEYCODE_SHIFT_LEFT ${SHIFTED_CODES[c]}`] }) &&
          buildAsciiKeyEventPlan(c, false) === null
        );
      }),
      RUN_OPTIONS
  ...
  test("a plan is gated by capability exactly for shift-requiring characters", () => {
    fc.assert(
      fc.property(shiftRequiring, c => buildAsciiKeyEventPlan(c, false) === null && buildAsciiKeyEventPlan(c, true) !== null),
      RUN_OPTIONS
    );
  });
  ...
  test("is total — null or a single well-formed `shell input ...` command", () => {
    fc.assert(
      fc.property(anyChar, fc.boolean(), (c, supports) => {
        ...
        return plan === null || (plan.commands.length === 1 && plan.commands[0].startsWith("shell input "));
      }),
      RUN_OPTIONS
    );
  });
  ...
  test("untypeable characters (control / non-ASCII) always yield null", () => {
    fc.assert(
      fc.property(untypeable, fc.boolean(), (c, supports) => buildAsciiKeyEventPlan(c, supports) === null),
      RUN_OPTIONS
    );
  });
});
