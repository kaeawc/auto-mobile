...
describe("buildAsciiKeyEventPlan", () => {
  test("digits map to direct key events", () => {
    expect(buildAsciiKeyEventPlan("4", false)).toEqual({ commands: ["shell input keyevent KEYCODE_4"] });
  });

  test("lowercase letters map to the uppercased key code without shift", () => {
    expect(buildAsciiKeyEventPlan("a", false)).toEqual({ commands: ["shell input keyevent KEYCODE_A"] });
  });
  ...
  test("direct punctuation maps without shift", () => {
    expect(buildAsciiKeyEventPlan("@", false)).toEqual({ commands: ["shell input keyevent KEYCODE_AT"] });
  });
