...
const asciiChar = fc.integer({ min: 0x20, max: 0x7e }).filter(c => c !== 0x60).map(c => String.fromCharCode(c));
...
const mixedText = fc.string({ unit: fc.oneof(asciiChar, fc.constantFrom(...SMART_CHARS)), maxLength: 24 });
...
describe("DefaultTextMatcher.partialTextMatch (property-based)", () => {
  test("is symmetric", () => {
    fc.assert(
      fc.property(mixedText, mixedText, fc.boolean(), (a, b, cs) =>
        matcher.partialTextMatch(a, b, cs) === matcher.partialTextMatch(b, a, cs)
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("is reflexive for non-empty text", () => {
    fc.assert(
      fc.property(mixedText.filter(s => s.length > 0), fc.boolean(), (a, cs) => matcher.partialTextMatch(a, a, cs)),
      RUN_OPTIONS
    );
  });
  ...
  test("an empty operand never matches", () => {
    fc.assert(
      fc.property(mixedText, a => !matcher.partialTextMatch(a, "") && !matcher.partialTextMatch("", a)),
      RUN_OPTIONS
    );
  });

  test("case-insensitive mode matches across letter casing", () => {
    const letters = fc.string({ unit: fc.constantFrom("a", "B", "c", "D", "e"), minLength: 1, maxLength: 10 });
    fc.assert(
...
describe("DefaultTextMatcher.createTextMatcher (property-based)", () => {
  test("an empty search text yields a matcher that never matches", () => {
    fc.assert(
      fc.property(fc.option(asciiText, { nil: undefined }), input => matcher.createTextMatcher("")(input) === false),
      RUN_OPTIONS
    );
  });
  ...
  test("a partial matcher agrees with normalized substring containment", () => {
    fc.assert(
      fc.property(mixedText.filter(s => s.length > 0), mixedText, (text, input) => {
        const fn = matcher.createTextMatcher(text, true, false);
        const expected = normalizeQuotes(input).toLowerCase().includes(normalizeQuotes(text).toLowerCase());
  ...
  test("a partial matcher matches its own (non-empty) search text", () => {
    fc.assert(
      fc.property(mixedText.filter(s => s.length > 0), text => matcher.createTextMatcher(text, true, false)(text)),
      RUN_OPTIONS
    );
  });
});
