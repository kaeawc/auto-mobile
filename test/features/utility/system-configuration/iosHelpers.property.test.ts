...
const segment = fc.string({ unit: fc.constantFrom("e", "n", "z", "H", "a", "C", "U", "S"), minLength: 1, maxLength: 4 });
const languageTag = fc.array(segment, { minLength: 1, maxLength: 4 }).map(parts => parts.join("-"));
  ...
  test("every entry is a hyphen-segment prefix of the tag", () => {
    fc.assert(
      fc.property(languageTag, tag => {
        ...
        return buildAppleLanguages(tag).every(entry => prefixes.has(entry) && tag.startsWith(entry));
      }),
      RUN_OPTIONS
    );
  });
...
describe("parseAppleTimeFormatRaw (property-based)", () => {
  test("maps \"1\"->\"24\" and \"0\"->\"12\"", () => {
    fc.assert(
      fc.property(fc.constant(null), () => parseAppleTimeFormatRaw("1") === "24" && parseAppleTimeFormatRaw("0") === "12"),
      RUN_OPTIONS
    );
  });

  test("passes any other value (including null) through unchanged", () => {
    const other = fc.option(fc.string({ maxLength: 6 }).filter(s => s !== "1" && s !== "0"), { nil: null });
    fc.assert(
  ...
  test("is idempotent", () => {
    ...
    fc.assert(
      fc.property(anyRaw, raw => parseAppleTimeFormatRaw(parseAppleTimeFormatRaw(raw)) === parseAppleTimeFormatRaw(raw)),
      RUN_OPTIONS
    );
  });
});
...
describe("iosSpawnCommand (property-based)", () => {
  test("composes `xcrun simctl spawn <udid> <command>` verbatim", () => {
    fc.assert(
      fc.property(fc.string({ maxLength: 40 }), fc.string({ maxLength: 40 }), (deviceId, command) => {
        const line = iosSpawnCommand(deviceId, command);
        return line === `xcrun simctl spawn ${deviceId} ${command}` && line.startsWith("xcrun simctl spawn ");
      }),
      RUN_OPTIONS
    );
  });
