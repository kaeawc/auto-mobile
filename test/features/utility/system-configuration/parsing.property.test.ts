...
describe("normalizeSettingValue (property-based)", () => {
  ...
  test("is idempotent", () => {
    fc.assert(
      fc.property(anyValue, v => normalizeSettingValue(normalizeSettingValue(v)) === normalizeSettingValue(v)),
      RUN_OPTIONS
    );
  });
...
describe("normalizeTimeFormat (property-based)", () => {
  ...
  test("accepts \"12\"/\"24\" with surrounding whitespace and rejects other tokens", () => {
    ...
    fc.assert(
      fc.property(valid, invalid, (good, bad) =>
  ...
  test("parses 1/true as true and 0/false as false, case- and whitespace-insensitively", () => {
    fc.assert(
      fc.property(fc.oneof(mixedCase("true"), fc.constant("1")), fc.oneof(mixedCase("false"), fc.constant("0")), (t, f) =>
        parseBooleanSetting(` ${t} `) === true && parseBooleanSetting(` ${f} `) === false
      ),
      RUN_OPTIONS
    );
  });
...
describe("parseLocaleList (property-based)", () => {
  const localeToken = fc.string({ unit: fc.constantFrom("e", "n", "U", "S", "f", "r", "-", " "), maxLength: 8 });
...
describe("extractCalendarFromLocale (property-based)", () => {
  ...
  const calId = fc.string({ unit: fc.constantFrom("a", "b", "g", "r", "0", "9"), minLength: 3, maxLength: 8 });
  ...
  test("the @calendar= keyword wins over a -u-ca- extension", () => {
    fc.assert(
      fc.property(calId, calId, (viaExt, viaKeyword) =>
  ...
  test("a locale with neither marker yields null", () => {
    const plainLocale = fc
      .tuple(fc.string({ unit: fc.constantFrom("e", "n", "f", "r"), minLength: 2, maxLength: 2 }),
