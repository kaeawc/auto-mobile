...
import { removeTempDbDir } from "../db/tempDbDir";
...
export async function useTempFileDatabase(): Promise<TempFileDatabaseHandle> {
  ...
  return {
    ...
    cleanup: async () => {
      ...
      // Use the single shared bounded-retry primitive (CLAUDE.md: one copy of
      // this Windows-lock cleanup). Its default give-up only fires on a
      // persistent lock (Windows), so it never trips the #2949 tripwire counter
      // off-Windows, where rm wins on the first attempt.
