...
// default. Pinned by test/parity/webrtcDeviceCaptureLatency.test.ts so the
// two cannot drift.
...
async function startWebRtcDaemon(
 ...
 */
async function ensureWebRtcDaemon(
  socketPath: string,
  daemonEnvironment: NodeJS.ProcessEnv,
): Promise<void> {
  try {
...
function chromeBinary(): string {
  ...
  if (!binary) {
    throw new Error("Chrome is required for WHEP decoding; set AUTOMOBILE_CHROME_BINARY");
  }
  return binary;
}
...
function chromeDiagnostics(chrome: ChildProcessWithoutNullStreams, logFile: string): string {
  ...
  try {
    ...
    if (contents) {
      tail = contents.slice(-1500);
    }
  } catch {
    /* the log may not exist if the spawn itself failed */
  }
...
async function videoSample(
  ...
  return response.result?.result?.value as {
    frames: number;
    width: number;
    height: number;
    sample: number;
...
async function waitForDecodedFrames(
  ...
  try {
    ...
    const diagnostics = await readerDiagnostics(cdp).catch(() => undefined);
    throw new Error(
      `${message}; reader diagnostics=${JSON.stringify(diagnostics ?? "unavailable")}`,
    );
  }
}
...
interface SimulatorAppearanceClient {
...
export function isBootedSimulatorState(device: { state: string } | null): boolean {
...
async function setIosFixtureAppearance(
  ...
  try {
    ...
    const udid = await waitForBootedSimulatorUdid(simctl, {
      timeoutMs: Math.min(IOS_SIMULATOR_BOOT_DISCOVERY_TIMEOUT_MS, timeoutMs),
      timer,
    });
    if (!udid) {
      // Appearance is cosmetic; video-frame assertions remain the lane's contract.
    ...
    // Appearance is cosmetic; video-frame assertions remain the lane's contract.
    console.warn(`[#6969] could not set iOS simulator ${appearance} appearance fixture: ${error}`);
  }
}
...
async function captureProfile(): Promise<CaptureProfile> {
  ...
  const usesVideoServer =
    Boolean(process.env.AUTOMOBILE_VIDEO_SERVER_JAR) ||
    process.env.AUTOMOBILE_REQUIRE_VIDEO_SERVER === "1";
    ...
    inFlight = sendWebRtcStreamRequest(
      ...
      .then((response) => {
        if (!response.success) {
          ...
          failure = new Error(
            `failed to renew WebRTC stream lease: ${response.error ?? "no error detail returned"}`,
          );
        }
      })
      .catch((error: unknown) => {
    ...
    async () => {
      ...
      try {
        ...
        startObserver = (async () => {
          while (
            observingStart &&
            !(timeline.has("whipConnected") && timeline.has("sourceStarted"))
          ) {
        ...
        if (!response.success) {
          throw new Error(
            `failed to start device WebRTC stream: ${response.error ?? "no error detail returned"}`,
          );
        }
        ...
        await waitForDecodedFrames(
          cdp,
          0,
          "browser did not decode device video on a static screen (late-viewer starvation, #4383)",
        );
        ...
        await changeFixture();
        const first = await waitForChangedSample(
          cdp,
          staticScreenFrame,
          "browser did not decode changed video after a visible change",
        );
        await launchFixture();
        ...
        const second = await waitForChangedSample(
          cdp,
          first,
          "browser did not receive changed video after returning to the fixture",
        );
