...
describe("archive extraction boundary (issue #4065)", () => {
  ...
  test("detects argv-first tar extraction regardless of flag position or order", () => {
    ...
    // Long-form --extract.
    expect(directlyExtractsTar('execFile("tar", ["--extract", "--file", archivePath]);')).toBe(
      true,
    );
  ...
  test("detects shell-string tar extraction, including static concatenation", () => {
    ...
    expect(directlyExtractsTar('const run = exec; run("tar -xzf archive.tar.gz -C dest");')).toBe(
      true,
    );
    expect(
      directlyExtractsTar('runExecSeam(cb, opts, { command: "tar", args: ["-xzf", archive] });'),
    ).toBe(true);
    expect(
      directlyExtractsTar(
    ...
    expect(
      directlyExtractsTar('runExecSeam(cb, opts, { command: "echo", args: ["tar", "-x"] });'),
    ).toBe(false);
    expect(
      directlyExtractsTar('exec(useTar ? "tar -xzf archive.tar.gz" : "unzip archive.zip");'),
    ).toBe(true);
    expect(directlyExtractsTar('exec(process.env.ARCHIVE_CMD ?? "tar -xzf archive.tar.gz");')).toBe(
      true,
    );
    expect(
      directlyExtractsTar(
        'import cp = require("node:child_process"); cp.exec("tar -xzf archive.tar.gz");',
      ),
    ).toBe(true);
  ...
  test("detects array-first tar extraction (Bun.spawn's single-array signature)", () => {
    ...
    expect(directlyExtractsTar('spawn(["tar", "-x", "-f", archivePath]);')).toBe(true);
    expect(
      directlyExtractsTar('Bun.spawn(["tar", "-C", dir, "--extract", "-f", archivePath]);'),
    ).toBe(true);
  ...
  test("follows const argv initializers and spreads (ordinary refactor forms)", () => {
    ...
    expect(directlyExtractsTar('const argv = ["tar", "-xzf", archive]; Bun.spawn(argv);')).toBe(
      true,
    );
    expect(directlyExtractsTar('const args = ["-xzf", archive]; spawn("tar", [...args]);')).toBe(
      true,
    );
    expect(directlyExtractsTar('const rest = ["-xzf", a]; Bun.spawn(["tar", ...rest]);')).toBe(
      true,
    );
    expect(
      directlyExtractsTar('const run = exec; run("tar", ["-xzf", archive], { cwd: dir });'),
    ).toBe(true);
    expect(
      directlyExtractsTar(
        'const prefix = dynamic; Bun.spawn([...prefix, "tar", "-xzf", archive]);',
      ),
    ).toBe(false);
  ...
  test("does not flag tar creation, listing, or non-extract options", () => {
    ...
    expect(directlyExtractsTar('spawn("tar", ["-czf", archive], { env: { MODE: "-x" } });')).toBe(
      false,
    );
    ...
    expect(
      directlyExtractsTar('executeCommand("tar", ["--exclude", pattern, "-czf", archivePath]);'),
    ).toBe(false);
  });
