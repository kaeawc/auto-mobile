...
function directlyExecutesDevicectl(source: string): boolean {
  if (!source.includes("devicectl") || !/(?:spawn|exec|execute|Bun)/i.test(source)) {
    return false;
  }
  ...
  return ast.calls.some((call) => {
    ...
    if (ast.isRunExecSeam(call)) {
      ...
      return (
        command.some((value) =>
          ast.strings(value).some((commandValue) => /(?:^|[/\\])xcrun$/.test(commandValue)),
    ...
    return alternatives.some(([command, ...arrayArgs]) => {
      ...
      const shellDevicectl = commandValues.some((value) =>
        /(?:^|[\s;&|])xcrun\s+devicectl(?:\s|$|[;&|])/.test(value),
      );
      ...
      const shellWrappedDevicectl = argvAlternatives.some((argv) => {
        const shellIndex = argv.findIndex((argument) =>
          ast.strings(argument).some((value) => /^-[a-z]*c[a-z]*$/i.test(value)),
        );
        return (
          shellIndex >= 0 &&
          ast
            .strings(argv[shellIndex + 1])
            .some((value) => /(?:^|[\s;&|])xcrun\s+devicectl(?:\s|$|[;&|])/.test(value))
        );
      });
      const argvWrappedDevicectl =
        commandValues.some((value) => value === "env" || value === "sudo") &&
        argvAlternatives.some((argv) => {
          ...
          while (commandIndex < values.length) {
            ...
            if (
              ["-u", "--user", "-g", "--group", "-h", "--host", "-C", "--close-from"].includes(
                value,
              )
            ) {
              commandIndex += 2;
          ...
          return (
            values[commandIndex] === "xcrun" && values.slice(commandIndex + 1).includes("devicectl")
          );
        });
      return (
        shellDevicectl ||
        shellWrappedDevicectl ||
        argvWrappedDevicectl ||
        directDevicectl ||
...
describe("devicectl execution boundary (issue #4053)", () => {
  ...
  test("detects execFile-style devicectl launches", () => {
    expect(
      directlyExecutesDevicectl('execFileAsync("xcrun", ["devicectl", "device", "info", "apps"]);'),
    ).toBe(true);
    expect(
    ...
    expect(directlyExecutesDevicectl('spawnSync("xcrun", ["devicectl", "--version"]);')).toBe(true);
    expect(
      directlyExecutesDevicectl(
  ...
  test("detects shell, direct-binary, and absolute xcrun devicectl forms", () => {
    ...
    expect(
      directlyExecutesDevicectl('const tool = "xcrun"; exec(`${tool} devicectl device list`);'),
    ).toBe(true);
    ...
    expect(
      directlyExecutesDevicectl('spawn("/usr/bin/xcrun", ["devicectl", "device", "list"]);'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl('spawn("/bin/sh", ["-c", "xcrun devicectl device list"]);'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl('Bun.spawn(["bash", "-c", "xcrun devicectl device list"]);'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl('spawn("bash", ["-lc", "xcrun devicectl device list"]);'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl('spawn("bash", (["-c", "xcrun devicectl device list"]));'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl('spawn("env", ["xcrun", "devicectl", "device", "list"]);'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl('execFile("sudo", ["xcrun", "devicectl", "device", "list"]);'),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl(
        'execFile("sudo", ["-u", "root", "xcrun", "devicectl", "device", "list"]);',
      ),
    ).toBe(true);
    expect(
      directlyExecutesDevicectl(
  ...
  test("does not treat unrelated execute methods as a process seam", () => {
    ...
    expect(directlyExecutesDevicectl('this.processExecutor.execute("xcrun", ["devicectl"]);')).toBe(
      true,
    );
  });
