...
describe("host shell execution boundary (issue #4068)", () => {
  test("rejects direct child_process shell APIs while allowing argv-first execution", () => {
    expect(findViolationsInSource("fixture.ts", 'import { exec } from "node:child_process"; exec("curl $HOST");')).toHaveLength(1);
    expect(findViolationsInSource("fixture.ts", 'const { exec } = await import("child_process"); exec("curl $HOST");')).toHaveLength(1);
  ...
  test("fetches the pull request base in shallow GitHub checkouts", () => {
    const calls: string[][] = [];
    let checks = 0;
    const baseRef = resolveBaseRef("origin/main", { GITHUB_ACTIONS: "true", GITHUB_BASE_REF: "main" }, (file, args) => {
      calls.push([file, ...args]);
      if (args[0] === "rev-parse" && checks++ === 0) {throw new Error("missing base");}
      return "";
    });

    expect(baseRef).toBe("origin/main");
    expect(calls).toContainEqual(["git", "fetch", "--no-tags", "--depth=1", "origin", "refs/heads/main:refs/remotes/origin/main"]);
  });
});
