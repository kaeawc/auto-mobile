...
type Branch = { properties?: Record<string, unknown>; required?: string[]; additionalProperties?: boolean };
const unionInput = fc
  .record({ combinator: fc.constantFrom("anyOf", "oneOf"), branches: fc.array(branch, { minLength: 1, maxLength: 4 }) })
  .map(({ combinator, branches }) => ({ schema: { [combinator]: branches } as Record<string, unknown>, branches: branches as Branch[] }));
...
describe("flattenTopLevelUnion (property-based)", () => {
  test("passes a schema without a top-level anyOf/oneOf array through unchanged", () => {
    const nonUnion = fc.oneof(
      fc.record({ type: fc.constant("object"), properties: fc.dictionary(propName, propSchema, { maxKeys: 3 }) }),
  ...
  test("flattens a union to an object schema with no combinator left", () => {
    fc.assert(
      fc.property(unionInput, ({ schema }) => {
        ...
        return r.type === "object" && typeof r.properties === "object" && !("anyOf" in r) && !("oneOf" in r);
      }),
      RUN_OPTIONS
    );
  });
  ...
  test("required is the intersection of the branches' required fields", () => {
    fc.assert(
      fc.property(unionInput, ({ schema, branches }) => {
        ...
        return expected.size === 0 ? !("required" in r) : setsEqual(new Set(r.required as string[]), expected);
      }),
      RUN_OPTIONS
    );
  });
  ...
  test("additionalProperties is set only when every branch that specifies it agrees", () => {
    fc.assert(
      fc.property(unionInput, ({ schema, branches }) => {
        const r = flattenTopLevelUnion(schema);
        const distinct = new Set(branches.map(b => b.additionalProperties).filter(x => typeof x === "boolean"));
  ...
  test("preserves a branch property named like an Object.prototype member (issue #4187)", () => {
    const protoKeyBranch = fc.record({
      type: fc.constant("object"),
      properties: fc.constant({ constructor: { type: "string" }, toString: { const: "x" }, id: { type: "number" } })
    });
    fc.assert(
