...
const primitive = fc.oneof(fc.string({ maxLength: 6 }), fc.integer(), fc.boolean(), fc.constant(null));
const nodeAt = (d: number): fc.Arbitrary<unknown> => {
  if (d <= 0) {
    return fc.oneof(primitive, boundsUnion);
  }
  const child = nodeAt(d - 1);
  const genericObject = fc.dictionary(fc.constantFrom("type", "title", "properties", "items", "foo"), child, { maxKeys: 3 });
...
const countBoundsUnions = (node: unknown): number => {
  ...
  return self + Object.values(node as Record<string, unknown>).reduce((s: number, v) => s + countBoundsUnions(v), 0);
};
