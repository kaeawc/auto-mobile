...
describe("buildNetworkGraph", () => {
  ...
  it("builds path tree", () => {
    ...
    expect(v1.paths["users"]).toBeDefined();
    expect(v1.paths["posts"]).toBeDefined();
  });
  ...
  it("computes stats correctly", () => {
    ...
    const result = buildNetworkGraph(events);
    const leaf = result.graph[0].paths["data"] as GraphLeaf;
  ...
  it("detects numeric IDs as parameterized", () => {
    ...
    expect(idNode.paths["posts"]).toBeDefined();
  });
  ...
  it("filters by minRequests", () => {
    ...
    expect(paths["popular"]).toBeDefined();
    expect(paths["rare"]).toBeUndefined();
  });
