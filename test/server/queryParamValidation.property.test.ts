...
const blank = fc.oneof(fc.constant(undefined), fc.string({ unit: fc.constantFrom(" ", "\t", "\n"), maxLength: 4 }));
...
describe("optionalInteger (property-based)", () => {
  test("round-trips an in-range integer", () => {
    const inRange = fc
      ...
      .chain(([min, span]) => fc.record({ min: fc.constant(min), max: fc.constant(min + span), n: fc.integer({ min, max: min + span }) }));
    fc.assert(
  ...
  test("throws below min (default 0) or above max", () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 1000 }), fc.integer({ min: 1, max: 500 }), (n, delta) => {
  ...
  test("throws on non-integer and non-numeric input", () => {
    ...
    const bad = fc.oneof(
      fc.double({ min: 0.01, max: 999.99, noNaN: true }).filter(x => !Number.isInteger(x)).map(String),
...
describe("optionalEnum (property-based)", () => {
  ...
  test("round-trips an allowed value and yields undefined for blanks", () => {
    fc.assert(
      fc.property(fc.constantFrom(...allowed), blank, (member, b) =>
        optionalEnum(member, "p", allowed) === member && optionalEnum(b, "p", allowed) === undefined
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("throws for a non-member", () => {
    const nonMember = fc.string({ minLength: 1, maxLength: 8 }).filter(s => s.trim().length > 0 && !allowed.includes(s.trim() as typeof allowed[number]));
    fc.assert(
...
describe("optionalBoolean (property-based)", () => {
  ...
  test("parses true/1 -> true and false/0 -> false, case-insensitively", () => {
    fc.assert(
      fc.property(fc.oneof(mixedCase("true"), fc.constant("1")), fc.oneof(mixedCase("false"), fc.constant("0")), (t, f) =>
...
describe("queryParamsToRecord (property-based)", () => {
  const key = fc.string({ unit: fc.constantFrom("a", "b", "c", "k", "x", "y"), minLength: 1, maxLength: 4 });
  ...
  test("round-trips a unique-key record through URLSearchParams", () => {
    fc.assert(
      fc.property(fc.dictionary(key, val, { maxKeys: 6 }), record => {
        ...
        return keys.length === Object.keys(parsed).length && keys.every(k => parsed[k] === record[k]);
      }),
      RUN_OPTIONS
    );
  });
