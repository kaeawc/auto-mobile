...
describe("session-scoped tool discovery", () => {
  ...
  test("retains a generated profile for an unbound stdio transport", () => {
    const binding = new SessionToolBinding(
      undefined,
      undefined,
      new FakeIdGenerator(["selection-profile-1"]),
    );
    const sessionUuid = binding.createAndBindToolSelectionProfile(undefined);

    expect(sessionUuid).toBe("selection-profile-1");
    expect(binding.effectiveSessionUuid(undefined)).toBeUndefined();
    expect(binding.effectiveToolSelectionProfileUuid(undefined)).toBe(sessionUuid);
    // A selection profile is connection state, not a releasable device session.
  ...
  test("binds a resumed profile to an unbound stdio transport without selecting device routing", () => {
    ...
    expect(binding.bindToolSelectionProfile(undefined, "selection-profile-1")).toBe(true);
    expect(binding.effectiveSessionUuid(undefined)).toBeUndefined();
    expect(binding.effectiveToolSelectionProfileUuid(undefined)).toBe("selection-profile-1");
  });
  ...
  test("seeds only a recreated transport's initial session binding", () => {
    ...
    expect(() =>
      binding.effectiveSessionUuid("recreated-transport", { sessionUuid: "device-session-b" }),
    ).toThrow("MCP connection is bound");
    expect(binding.effectiveSessionUuid("recreated-transport")).toBe("device-session-a");
  });

  test.each(["", "   "])(
    "uses the bound session when an explicit session UUID is %p",
    (sessionUuid) => {
      const binding = new SessionToolBinding();
