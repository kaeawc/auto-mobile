...
describe("isAdbMissingDeviceError / extractAdbMissingDeviceId", () => {
  const extractRows: Array<{ name: string; input: unknown; expected: string | null }> = [
    { name: "quoted device-not-found", input: new Error("error: device 'emulator-5554' not found"), expected: "emulator-5554" },
    { name: "case-insensitive Device Not Found", input: new Error("Device 'pixel-7' Not Found"), expected: "pixel-7" },
    { name: "serial with colon/port", input: new Error("device '127.0.0.1:5555' not found"), expected: "127.0.0.1:5555" },
    { name: "generic no-quote message has no id", input: new Error("device not found"), expected: null },
  ...
  const matchRows: Array<{ name: string; input: unknown; expected?: string; result: boolean }> = [
    { name: "quoted id matches expected device", input: new Error("device 'emulator-5554' not found"), expected: "emulator-5554", result: true },
    { name: "quoted id mismatched against expected device", input: new Error("device 'emulator-5554' not found"), expected: "other", result: false },
    { name: "quoted id with no expectation", input: new Error("device 'emulator-5554' not found"), result: true },
    { name: "generic 'device not found' with no expectation", input: new Error("device not found"), result: true },
    { name: "generic 'no devices' with no expectation", input: new Error("adb: no devices/emulators found"), result: true },
    { name: "generic message but a specific device expected", input: new Error("device not found"), expected: "emulator-5554", result: false },
    { name: "unrelated error is never a missing-device error", input: new Error("offline"), result: false },
  ];
