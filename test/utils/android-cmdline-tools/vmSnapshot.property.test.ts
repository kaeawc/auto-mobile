...
const asResult = (stdout: string, stderr: string): ExecResult => ({ stdout, stderr }) as unknown as ExecResult;

// Output lines biased toward the OK/KO tokens (and near-misses like OKAY/BOOK).
const line = fc.oneof(
  fc.constantFrom("OK", "KO", "ok", "ko", "OKAY", "BOOK", "KO: failed", "snapshot OK", "done", "error", ""),
...
const evalOracleOk = (stdout: string, stderr: string): boolean => {
  const combined = [stdout, stderr].filter(p => p && p.trim().length > 0).join("\n").trim().toUpperCase();
...
describe("evaluateVmSnapshotResult (property-based)", () => {
  test("ok is true exactly when OK is present and KO is absent (word-boundary)", () => {
    fc.assert(
      fc.property(action, snapshotName, line, line, (a, name, stdout, stderr) => {
        return evaluateVmSnapshotResult(a, name, asResult(stdout, stderr)).ok === evalOracleOk(stdout, stderr);
      }),
      RUN_OPTIONS
    );
  });
  ...
  test("KO takes precedence over OK", () => {
    fc.assert(
      fc.property(action, snapshotName, (a, name) => {
        return evaluateVmSnapshotResult(a, name, asResult("OK", "KO")).ok === false &&
...
describe("formatVmSnapshotExecutionError (property-based)", () => {
  test("always opens with the failed-snapshot base line", () => {
    fc.assert(
      fc.property(action, snapshotName, fc.oneof(fc.string({ maxLength: 20 }), fc.string().map(m => new Error(m))), (a, name, error) => {
        return formatVmSnapshotExecutionError(a, name, error).startsWith(`VM snapshot ${a} failed for '${name}'`);
      }),
      RUN_OPTIONS
    );
  });
