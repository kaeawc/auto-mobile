...
describe("PlistClient", () => {
  test("converts a file to structured JSON through literal argv", async () => {
    ...
    const process: PlistProcess = async request => {
      calls.push(request);
      return { stdout: Buffer.from('{"CFBundleIdentifier":"com.example.app"}'), stderr: Buffer.alloc(0) };
    };
    ...
    expect(calls[0].args).toEqual(["-convert", "json", "-o", "-", "--", "/tmp/a;not-a-command/Info.plist"]);
  });
  ...
  test("converts bytes through stdin without a temporary file", async () => {
    ...
    const process: PlistProcess = async request => {
      ...
      return { stdout: Buffer.from('{"com.apple.Maps":{"bundleName":"Maps"}}'), stderr: Buffer.alloc(0) };
    };
  ...
  test("rejects malformed conversion output with actionable context", async () => {
    const process: PlistProcess = async () => ({ stdout: Buffer.from("not json"), stderr: Buffer.alloc(0) });

    await expect(new PlistClient(process).readJsonFile("/tmp/bad.plist"))
      .rejects.toThrow("plutil produced malformed JSON");
  });

  test("bounds oversized output before parsing", async () => {
    const process: PlistProcess = async () => ({ stdout: Buffer.alloc(17), stderr: Buffer.alloc(0) });
    const plist = new PlistClient(process, { maxOutputBytes: 16 });

    await expect(plist.readJsonFile("/tmp/large.plist")).rejects.toThrow("output exceeded 16 bytes");
  });

  test("reports unavailable when the owner process cannot start", async () => {
    const process: PlistProcess = async () => { throw new Error("ENOENT: plutil"); };
  ...
  test("aborts the injected child process on the owner timeout", async () => {
    ...
    let aborted = false;
    const process: PlistProcess = request => new Promise((_resolve, reject) => {
      request.signal?.addEventListener("abort", () => {
        aborted = true;
        reject(new Error("aborted"));
      }, { once: true });
    });
    const read = new PlistClient(process, { timeoutMs: 25, timer }).readJsonFile("/tmp/wedged.plist");
  ...
  test("forwards caller cancellation to the injected child process", async () => {
    ...
    const read = new PlistClient(process).readJsonFile("/tmp/cancelled.plist", { signal: controller.signal });
