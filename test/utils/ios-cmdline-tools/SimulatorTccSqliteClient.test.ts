...
describe("SimulatorTccSqliteClient", () => {
  ...
  test("encodes apostrophes and quotes for sqlite dot-command parameters", async () => {
    const executor = new FakeSqliteExecutor();
    executor.onExecute = async () => executor.calls.at(-1)?.args.at(-1) === "pragma table_info(access);"
      ? result(JSON.stringify([{ name: "service" }, { name: "client" }]))
      : result("[]");
    const client = new SimulatorTccSqliteClient({ executor, fileSystem: new FakeTccFileSystem(), homeDirectory: "/Users/tester" });
  ...
  test("supports legacy TCC schemas that expose allowed instead of auth_value", async () => {
    ...
    executor.onExecute = async () => executor.calls.at(-1)?.args.at(-1) === "pragma table_info(access);"
      ...
      : result(JSON.stringify([{ service: "kTCCServiceCamera", client: "com.example.app", allowed: 1 }]));
    const client = new SimulatorTccSqliteClient({ executor, fileSystem: new FakeTccFileSystem(), homeDirectory: "/Users/tester" });
  ...
  test("reports unavailable or unreadable TCC databases with device context", async () => {
    ...
    const client = new SimulatorTccSqliteClient({ executor: new FakeSqliteExecutor(), fileSystem, homeDirectory: "/Users/tester" });
  ...
  test("rejects a non-simulator UDID before resolving a host path", async () => {
    ...
    await expect(client.readPermissions("../../other-device", "com.example.app")).rejects.toThrow("requires a simulator UDID");
  ...
  test("classifies an unavailable sqlite3 binary and a malformed TCC database", async () => {
    ...
    await expect(client.readPermissions(DEVICE_ID, "com.example.app")).rejects.toThrow("sqlite3 is unavailable");
  ...
  test("rejects incompatible schemas and malformed sqlite JSON", async () => {
    ...
    await expect(client.readPermissions(DEVICE_ID, "com.example.app")).rejects.toThrow("missing required access columns: service");

    executor.response = result("not json");
    await expect(client.readPermissions(DEVICE_ID, "com.example.app")).rejects.toThrow("sqlite3 returned malformed JSON");
  });
  ...
  test("aborts the argv execution when the owned timeout elapses", async () => {
    ...
    executor.onExecute = async options => new Promise<ExecResult>((_resolve, reject) => {
      capturedSignal = options?.signal;
      capturedSignal?.addEventListener("abort", () => reject(new Error("The operation was aborted")), { once: true });
    });
    const client = new SimulatorTccSqliteClient({
      executor,
    ...
    await expect(pending).rejects.toThrow("Timed out after 123ms while reading simulator TCC database");
