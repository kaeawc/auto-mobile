...
const hex = (n: number): fc.Arbitrary<string> =>
  fc.array(fc.integer({ min: 0, max: 15 }), { minLength: n, maxLength: n }).map(a => a.map(x => x.toString(16)).join(""));
...
const androidSerial = fc.string({ unit: fc.constantFrom("a", "Z", "0", "9", "x", "7"), maxLength: 20 });
const adbShape = fc.oneof(
  fc.integer({ min: 5554, max: 5680 }).map(p => `emulator-${p}`),
  fc.tuple(fc.integer({ min: 0, max: 255 }), fc.integer({ min: 1024, max: 65535 })).map(([h, p]) => `192.168.1.${h}:${p}`),
...
describe("iosDeviceType (property-based)", () => {
  ...
  test("simulator and physical classifications are mutually exclusive for any input", () => {
    fc.assert(
      fc.property(fc.oneof(simUuid, physModern, physLegacy, androidSerial, fc.string({ maxLength: 45 })), id =>
        !(isIosSimulatorUdid(id) && isIosPhysicalUdid(id))
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("isIosUdid is exactly the disjunction of the two specific predicates", () => {
    fc.assert(
      fc.property(fc.oneof(simUuid, physModern, physLegacy, fc.string({ maxLength: 45 })), id =>
        isIosUdid(id) === (isIosSimulatorUdid(id) || isIosPhysicalUdid(id))
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("Android serials and other adb id shapes are never classified as iOS (issue #4165)", () => {
    fc.assert(
      fc.property(fc.oneof(androidSerial, adbShape), id =>
        !isIosSimulatorUdid(id) && !isIosPhysicalUdid(id) && !isIosUdid(id)
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("classification is case-insensitive", () => {
    fc.assert(
      fc.property(fc.oneof(simUuid, physModern, physLegacy), id =>
        isIosUdid(id) === isIosUdid(id.toUpperCase()) && isIosUdid(id) === isIosUdid(id.toLowerCase())
      ),
      RUN_OPTIONS
    );
  });
  ...
  test("is total — a boolean for arbitrary strings, never throwing", () => {
    fc.assert(
      fc.property(fc.string({ maxLength: 50 }), id =>
        typeof isIosSimulatorUdid(id) === "boolean" && typeof isIosPhysicalUdid(id) === "boolean" && typeof isIosUdid(id) === "boolean"
      ),
      RUN_OPTIONS
    );
  });
