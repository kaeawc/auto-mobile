  ...
  test("daemon-launch-<dead pid>.log older than the stale threshold is swept once its owner has exited", async () => {
    await withTempLogDir(async (dir) => {
      ...
      await pruneLogFiles({
        ...
        // Future clock so age is deterministically stale regardless of Windows
        // filesystem/wall-clock precision differences.
        now: Date.now() + 60_000,
...
describe("logPruner daemon-launch inherited-fd guard (issue #6194)", () => {
  ...
  test("still prunes a non-launch dead peer's stale log even while a daemon is running", async () => {
    await withTempLogDir(async (dir) => {
      ...
      const peerStats = await stat(path.join(dir, stalePeer));
      const launchStats = await stat(path.join(dir, launchLog));
      ...
      await pruneLogFiles({
        ...
        // Derive the sweep clock from recorded mtimes so both files are stale
        // regardless of Windows filesystem and wall-clock precision differences.
        now: Math.max(peerStats.mtimeMs, launchStats.mtimeMs) + 2000,
  ...
  test("prunes the launch log once NO namespace has a live daemon", async () => {
    await withTempLogDir(async (dir) => {
      ...
      await pruneLogFiles({
        ...
        maxOwnFiles: 10,
        abandonedMaxAgeMs: -1,
        now: Date.now() + 60_000, // deterministically stale on any filesystem clock
  ...
  test("retries a transient EBUSY and still removes the launch log and its tombstone", async () => {
    await withTempLogDir(async (dir) => {
      ...
      await pruneLogFiles({
        ...deadNamespace,
        dir,
        // Future clock so the stale precondition holds regardless of Windows
        // filesystem/wall-clock precision differences.
        now: Date.now() + 60_000,
