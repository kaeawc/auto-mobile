...
describe("formatMcpServerVersion (property-based)", () => {
  test("stamping then stripping recovers the base version (round-trip)", () => {
    fc.assert(
      fc.property(baseVersion, gitInfo, (base, git) => releaseVersion(formatMcpServerVersion(base, git)) === base),
      RUN_OPTIONS
    );
  });
  ...
  test("no git info (or an empty SHA) leaves the base unchanged", () => {
    ...
    fc.assert(
      fc.property(baseVersion, emptyOrNull, (base, git) => formatMcpServerVersion(base, git) === base),
      RUN_OPTIONS
    );
  });
  ...
  test("a real SHA prefixes the stamp and reflects the dirty marker exactly", () => {
    fc.assert(
      fc.property(baseVersion, gitInfo, (base, git) => {
        ...
        return stamped.startsWith(`${base}+g${git.shortSha}`) && stamped.includes(".dirty") === git.dirty;
      }),
      RUN_OPTIONS
    );
  });
...
describe("resolveMcpServerVersion (property-based)", () => {
  test("an explicit MCP_SERVER_VERSION override is returned verbatim, never stamped", () => {
    ...
    fc.assert(
      fc.property(override, fc.option(baseVersion, { nil: undefined }), gitInfo, (value, npm, git) => {
        return (
          resolveMcpServerVersion({
  ...
  test("with no override and no resolvable base, the result is 'unknown'", () => {
    fc.assert(
      fc.property(fc.constantFrom("", undefined), fc.constantFrom("", undefined), gitInfo, (override, npm, git) => {
        return (
          resolveMcpServerVersion({
