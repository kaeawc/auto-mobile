...
describe("parseOutputReductionFlags (property-based)", () => {
  ...
  test("a single CLI flag enables exactly its own field", () => {
    fc.assert(
      fc.property(spec, s => {
        ...
        return OUTPUT_REDUCTION_FLAG_SPECS.every(other => flags[other.field] === (other.field === s.field));
      }),
      RUN_OPTIONS
    );
  });
  ...
  test("a present CLI flag wins over a disabling env var", () => {
    fc.assert(
      fc.property(spec, fc.string({ maxLength: 4 }).filter(v => v !== "1"), (s, disabling) => {
  ...
  test("emits exactly the CLI flags for the truthy fields, in spec order", () => {
    fc.assert(
      fc.property(flagsArb, flags => {
        ...
        const expected = OUTPUT_REDUCTION_FLAG_SPECS.filter(s => flags[s.field]).map(s => s.cli);
...
describe("outputReductionFlags round-trip (property-based)", () => {
  test("parse ∘ toArgs recovers a full flag record exactly", () => {
    fc.assert(
      fc.property(flagsArb, flags => fieldsEqual(parseOutputReductionFlags(outputReductionFlagsToArgs(flags), {}), flags)),
      RUN_OPTIONS
    );
  });
  ...
  test("toArgs ∘ parse is a canonical fixed point over arbitrary argv", () => {
    ...
    const argv = fc.array(fc.oneof(spec.map(s => s.cli), fc.string({ maxLength: 8 })), { maxLength: 15 });
    fc.assert(
