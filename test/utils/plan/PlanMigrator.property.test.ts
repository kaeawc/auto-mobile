...
const wellFormedVersion = fc
  .tuple(versionPart, versionPart, versionPart)
  .map(([major, minor, patch]) => `${major}.${minor}.${patch}`);

const dirtyBuildVersion = fc
  .tuple(versionPart, versionPart, versionPart)
  .map(([major, minor, patch]) => `${major}.${minor}.${patch}+gabc123.dirty`);
...
const anyVersionLike = fc.oneof(wellFormedVersion, dirtyBuildVersion, fc.string());
